...
function anchorJump2(l) {
    var text = $(l).text();
    var text1 = parseInt(text);
    if (text.length < 4) {$('.letter').text(text)
        $('.letter').css({'opacity':' 1'})
        location.hash = '#s' + text1
    }
}
function touchEnd() {
    var opcityNum = 1;
...
function move1() {
    event.preventDefault();
    anchorJump1(document.elementFromPoint(event.changedTouches[0].clientX,event.changedTouches[0].clientY))
}
function move2() {
    event.preventDefault();
    anchorJump2(document.elementFromPoint(event.changedTouches[0].clientX,event.changedTouches[0].clientY))
}