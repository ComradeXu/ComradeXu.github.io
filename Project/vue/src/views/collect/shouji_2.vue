<template>
  <div>
    ...
    <el-container class="cotent">
      <el-main class="elTable">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <a href="javascript:;" @click="onefriendlys(scope.row)">添加配置</a>
              <a href="javascript:;" @click="caiji(scope.row)">{{scope.row.cjstatus}}</a>
    ...
    <el-dialog title="添加配置" :visible.sync="centerDialogVisibles" width="680px" center>
      <el-form ref="ruleForm" :model="ruleForms" :rules="ruleas" label-width="110px">
        <el-row>
          <el-col :span="14">
            <el-form-item label="配置名" prop="name">
              <el-input v-model="ruleForms.name" placeholder="请输入配置名称" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="排序" prop="orders">
              <el-input v-model="ruleForms.orders" placeholder="请输入排序" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="14">
            <el-form-item label="父级" prop="parentid">
              <el-input v-model="ruleForms.parentid" placeholder="请输入父级ID" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="分页" prop="pagenum">
              <el-input v-model="ruleForms.pagenum" placeholder="请输入分页" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="14">
            <el-form-item label="采集URL" prop="URL">
              <el-input v-model="ruleForms.url" placeholder="请输入URL" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="标签名" prop="tagname">
              <el-input v-model="ruleForms.tagname" placeholder="请输入标签名" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="14">
            <el-form-item label="类名" prop="classname">
              <el-input v-model="ruleForms.classname" placeholder="请输入类名" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="二级标签名" prop="ejclasssname">
              <el-input v-model="ruleForms.ejclassname" placeholder="请输入二级标签名" />
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="14">
            <el-form-item label="内容页标签" prop="tagcontent">
              <el-input v-model="ruleForms.tagcontent" placeholder="请输内容页" />
            </el-form-item>
          </el-col>
          <el-col :span="10">
            <el-form-item label="内容标签名" prop="content">
              <el-input v-model="ruleForms.content" placeholder="请输入内容标签名" />
            </el-form-item>
          </el-col>
        </el-row>
      </el-form>
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="addSet('ruleForm')">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
export default {
  data () {
    return {
      ...
      ruleas: {
        name: [{ required: true,message: '请填写栏目',trigger: 'blur'}],
        orders: [{ required: true,message: '请填写排序',trigger: 'blur'}],
        parentid: [{ required: true,message: '请填写父级id',trigger: 'blur'}],
        pagenum: [{ required: true,message: '请填写分页',trigger: 'blur'}],
        url: [{ required: true,message: '请填写采集地址',trigger: 'blur'}],
        tagname: [{ required: true,message: '请填写标签名',trigger: 'blur'}],
        classname: [{ required: true,message: '请填写类名',trigger: 'blur'}],
        ejclassname: [{ required: true,message: '请填写二级标签名',trigger: 'blur'}],
        tagcontent: [{ required: true,message: '请填写内容页标签',trigger: 'blur'}],
        contentclassname: [{ required: true,message: '请填写内容页类名',trigger: 'blur'}]
      },
      sHeight: sHeight_c,
      loading: false,
      name: '',
  ...
  methods: {
    ...
    fetchData () {
      this.loading = true
      getquanzhong(this.pagesObj.page, this.pagesObj.page_num, this.parentid).then(res => {
    ...
    onefriendlys (row) {
      ...
      this.ruleForms.parentid = row.id
      this.fetchData()
    },
    addSet (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          addconf(this.ruleForms).then(res => {
    ...
    addfriendlys (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          addlanmu(this.ruleForm, this.parentid).then(res => {
            this.centerDialogVisible = false
            this.$message({
              message: '添加成功',
              type: 'success'
